...
**Install** — pick your host. Every option installs the same skills from the same
...
make check    # exactly what CI runs: build, drift, validate, manifests, install smoke test
```

Edit `source/`, never `skills/` — the latter is generated by `make build`, and CI fails on drift.
New skills, sharper checks, and better anti-pattern descriptions are all welcome; see
[CONTRIBUTING.md](CONTRIBUTING.md) for how to write a check that actually changes behaviour, and
[PROJECT.md](PROJECT.md) for the conventions the validator enforces.
